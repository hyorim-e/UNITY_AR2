...
public class RegisterAccount : LoginBase
{
    ...
    public void OnClickRegisterAccount()
    {
        ...
        if (IsFieldDataEmpty(imagePW, inputFieldID.text, "비밀번호")) return;
    ...
    {
        Backend.BMember.CustomSignUp(inputFieldID.text, inputFieldConfirmPW.text, callback =>
        {
            // "계정 생성" 버튼 상호작용 활성화
            ...
            if (callback.IsSuccess())
            {
                ...
                Backend.BMember.UpdateCustomEmail(inputFieldEmail.text, callback =>
                {
                    if (callback.IsSuccess())
                    {
                        SetMessage($"계정 생성 성공. {inputFieldID}님 환영합니다.");

                        // 계정 생성에 성공했을 때 해당 계정의 게임 정보 생성
                        BackendGameData.Instance.GameDataInsert();

                        // Lobby 씬으로 이동
                        Utills.LoadScene(SceneNames.Lobby);
                    }
                });
            }
            //게정 생성 실패
